...
export default function ManageAdminsPage() {
  ...
  async function handleAddAdmin(e: React.FormEvent) {
    ...
    const result = await authedFetch("/api/admin/create-admin", { email: adminEmail, role: adminRole });
    ...
    setNotice(
      (result.data as { promoted?: boolean })?.promoted
        ? `${adminEmail} was already a contributor — promoted to ${adminRole}.`
  ...
  async function handleAddContributor(e: React.FormEvent) {
    ...
    setNotice(`Invite sent to ${contribEmail}.`);
    setContribName("");
    setContribHandle("");
    setContribEmail("");
    loadAll();
    ...
    <div className="text-white space-y-10 max-w-4xl">
      ...
      <section className="space-y-3">
        ...
        <form onSubmit={handleAddAdmin} className="space-y-3 lv-card-flush p-4">
          <p className="text-sm text-white/60">
            Sends an email invite — the new admin sets their own password from the link. If the email already belongs to an
            approved contributor, their account is promoted instead (no new invite needed).
          </p>
          <div className="flex gap-2 flex-wrap">
            ...
            <button type="submit" disabled={addingAdmin} className="lv-btn-primary">
              {addingAdmin ? "Inviting..." : "Add Admin"}
            </button>
          </div>
        </form>
      ...
      <section className="space-y-3">
        ...
        <form onSubmit={handleAddContributor} className="space-y-3 lv-card-flush p-4">
          <p className="text-sm text-white/60">Add a contributor directly — skips the application step, sends an email invite.</p>
          <div className="flex gap-2 flex-wrap">
            <input
              required
              value={contribName}
            ...
            <button type="submit" disabled={addingContrib} className="lv-btn-primary">
              {addingContrib ? "Inviting..." : "Add Contributor"}
            </button>
          </div>
        </form>
